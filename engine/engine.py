    ...
    def pymunk_moved(self, physics_engine, dx, dy, d_angle):
        ...
        if len(arcade.check_for_collision_with_list(self, self.ladder_list)) > 0:
            if not self.is_on_ladder:
                ...
                self.pymunk.max_vertical_velocity = self.MAX_HORIZONTAL_SPEED
        else:
            if self.is_on_ladder:
                self.pymunk.damping = 1.0
                self.pymunk.max_vertical_velocity = self.MAX_VERTICAL_SPEED
                self.is_on_ladder = False
        ...
        if self.is_on_ladder and not is_on_ground:
            ...
            if abs(self.y_odometer) > self.DISTANCE_TO_CHANGE_TEXTURE:
                # Reset the odometer
                self.y_odometer = 0
...
class GameWindow(arcade.Window):
    ...
    def setup(self):
        ...
        sprite2 = PlayerSprite(
            self.ladder_list, hit_box_algorithm=arcade.hitbox.algo_detailed
        )
        sprite2.center_x = SPRITE_SIZE * 10 + SPRITE_SIZE / 2
        sprite2.center_y = SPRITE_SIZE * 10 + SPRITE_SIZE / 2
        self.player_list.append(sprite2)

        # --- Pymunk Physics Engine Setup ---

        mode = 'side'
        view_modes = {
            'topdown': {'damping': 0.1, 'gravity': (0, 0)},
            'side': {'damping': 1.0, 'gravity': (0, -1500)},
            }
        self.physics_engine = arcade.PymunkPhysicsEngine(**view_modes[mode])
    ...
    def on_key_press(self, key, modifiers):
        ...
        if key in (arcade.key.LEFT, arcade.key.A):
            ...
            if (
                ...
                impulse = (0, self.player_sprite.JUMP_IMPULSE)
    ...
    def on_update(self, delta_time):
        ...
        for sprite in self.player_list:
            is_on_ground = self.physics_engine.is_on_ground(sprite)
            # Update player forces based on keys pressed
            if self.left_pressed and not self.right_pressed:
                # Create a force to the left. Apply it.
                if is_on_ground or sprite.is_on_ladder:
                    force = (-sprite.MOVE_FORCE_ON_GROUND, 0)
                else:
                    force = (-sprite.MOVE_FORCE_IN_AIR, 0)
                ...
                # Create a force to the right. Apply it.
                if sprite.is_on_ladder:
                    force = (0, sprite.MOVE_FORCE_ON_GROUND)
                    self.physics_engine.apply_force(sprite, force)
                    # Set friction to zero for the player while moving
                    self.physics_engine.set_friction(sprite, 0)
