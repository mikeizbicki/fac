...
cp "$text_file" "$text_file.stripped"

# remove markdown images
sed -i '/!\[.*\](.*)/d' "$text_file.stripped"

# remove Vocab section
sed -i '/## Vocabulary/,/## Grammar/{ /## Grammar/!d }' "$text_file.stripped"

echo "stripped token count=$(ttok "$(cat "$text_file.stripped")" 2> /dev/null)"
